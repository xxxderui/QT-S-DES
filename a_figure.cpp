#include "a_figure.h"
#include "ui_a_figure.h"
#include "choice.h"

A_figure::A_figure(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::A_figure)
{
    ui->setupUi(this);
...
// 初始化静态成员变量
const int SDES_2::IP[8] = { 2, 6, 3, 1, 4, 8, 5, 7 };
const int SDES_2::EP[8] = { 4, 1, 2, 3, 2, 3, 4, 1 };
const int SDES_2::P4[4] = { 2, 4, 3, 1 };
const int SDES_2::IP_INV[8] = { 4, 1, 3, 5, 7, 2, 8, 6 };
const int SDES_2::P10[10] = { 3, 5, 2, 7, 4, 10, 1, 9, 8, 6 };
const int SDES_2::P8[8] = { 6, 3, 7, 4, 8, 5, 10, 9 };
const int SDES_2::S0[4][4] = { {1, 0, 3, 2}, {3, 2, 1, 0}, {0, 2, 1, 3}, {3, 1, 0, 2} };
const int SDES_2::S1[4][4] = { {0, 1, 2, 3}, {2, 3, 1, 0}, {3, 0, 1, 2}, {2, 1, 0, 3} };

// 实现各种成员函数

// 实现新函数：暴力破解
vector<int> SDES_2::bruteForceDecrypt(const vector<int>& ciphertext, const vector<int>& knownPlaintext) {
...
vector<int> SDES_2::asciiToBinary(const string& input) {
...
string SDES_2::binaryToAscii(const vector<int>& input) {
    string asciiData;
...
string SDES_2::encryptAscii(const string& plaintext, const vector<int>& key) {
...
string SDES_2::decryptAscii(const string& ciphertext, const vector<int>& key) {
...
// 辅助函数：将字符串向量连接成一个字符串
string SDES_2::join(const vector<string>& strings, const string& delimiter) {
    string result;
...
//-----------------------分割线--------------------------//
    ...
    // P4置换函数
    vector<int> SDES_2::permuteP4(const vector<int>& input) {
        vector<int> output(4);
        for (int i = 0; i < 4; i++) {
    ...
    // P10置换
    vector<int> SDES_2::permuteP10(const vector<int>& key) {
    ...
    // P8置换
    vector<int> SDES_2::permuteP8(const vector<int>& key) {
        vector<int> result(8);
        for (int i = 0; i < 8; i++) {
            result[i] = key[P8[i] - 1];
        }
    ...
    // 扩展函数 EP
    vector<int> SDES_2::expandEP(const vector<int>& input) {
        vector<int> output(8);
        for (int i = 0; i < 8; i++) {
    ...
    // 两个位数组进行异或操作
    vector<int> SDES_2::xorOperation(const vector<int>& a, const vector<int>& b) {
    ...
    // S-盒替换
    vector<int> SDES_2::sBox(const vector<int>& input, const int sBoxTable[4][4]) {
    ...
    // F函数
    vector<int> SDES_2::fFunction(const vector<int>& r, const vector<int>& subKey) {
    ...
    // 循环左移
    vector<int> SDES_2::circularLeftShift(const vector<int>& input) {
    ...
    // 连接两个位数组
    vector<int> SDES_2::concat(const vector<int>& a, const vector<int>& b) {
    ...
    // 初始置换（IP）
    vector<int> SDES_2::initialPermutation(const vector<int>& input) {
        vector<int> output(8);
        for (int i = 0; i < 8; i++) {
    ...
    // 逆初始置换（IP^-1）
    vector<int> SDES_2::inverseInitialPermutation(const vector<int>& input) {
        vector<int> output(8);
        for (int i = 0; i < 8; i++) {
    ...
    // 密钥生成
    vector<vector<int>> SDES_2::generateSubKeys(const vector<int>& key) {
    ...
    // 使用10位密钥加密8位明文
    vector<int> SDES_2::encrypt(const vector<int>& plaintext, const vector<int>& key) {
    ...
    // 使用10位密钥解密8位密文
    vector<int> SDES_2::decrypt(const vector<int>& ciphertext, const vector<int>& key) {
        vector<vector<int>> subKeys = generateSubKeys(key);
...
void A_figure::mousePressEvent(QMouseEvent *e)
{
...
void A_figure::on_pushButton_clicked()
{
    ...
    if(text1.length()==0)
    {
        ...
        else
        {
            ...
            if(w)
            {
                ...
                string encrypted=text3.toStdString();
                string decrypted = SDES_2::decryptAscii(encrypted, key);
                QString qstr = QString::fromStdString(decrypted);
                qDebug()<<QString::fromStdString(encrypted);
    ...
    else
    {
        ...
        else
        {
            ...
            if(w)
            {
                ...
                string encrypted = SDES_2::encryptAscii(plaintext, key);
                QString qstr = QString::fromStdString(encrypted);
